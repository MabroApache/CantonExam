...
onMounted(async () => {
  try {
    const questions = await questionApi.getByCreatorId(userInfo.value.id)
    stats.value.questionCount = questions.data?.length || 0

    const papers = await paperApi.getByCreatorId(userInfo.value.id)
    stats.value.paperCount = papers.data?.length || 0

    const examList = await examApi.search({ creatorId: userInfo.value.id })

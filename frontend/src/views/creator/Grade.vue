  ...
  <div class="creator-grade">
    ...
    <el-dialog title="阅卷打分" v-model="gradeDialogVisible" width="900px">
      <el-descriptions :column="2" border>
        ...
        <el-descriptions-item label="考生姓名">{{ gradeRecord.candidateName }}</el-descriptions-item>
      ...
      <div v-for="(answer, index) in subjectiveAnswers" :key="answer.id" class="grade-item">
        ...
        <div class="grade-content">
          ...
          <div class="candidate-answer">{{ answer.candidateAnswer || '未作答' }}</div>
    ...
    <el-dialog title="试卷详情" v-model="viewDialogVisible" width="800px">
      <el-descriptions :column="2" border>
        ...
        <el-descriptions-item label="考生姓名">{{ viewRecord.candidateName }}</el-descriptions-item>
...
const handleGrade = async (row) => {
  ...
  try {
    ...
    objectiveAnswers.value = answers.filter(a => a.typeName === '单选题' || a.typeName === '判断题' || a.typeName === '多选题')
    subjectiveAnswers.value = answers.filter(a => a.typeName === '填空题' || a.typeName === '简答题')
...
const handleSaveGrade = async () => {
  loading.value = true
  try {
    const answers = [...objectiveAnswers.value, ...subjectiveAnswers.value]
    const gradeData = {

...
const loadExamRecord = async () => {
  ...
  console.log('sessionStorage中的recordId:', recordId)
  ...
  if (!examRecord.value) {
    console.log('创建新的考试记录，参数:', {
      ...
      candidateId: userInfo.value.id,
      candidateName: userInfo.value.name
    })
    ...
    examRecord.value = res.data
    sessionStorage.setItem('currentRecordId', res.data.id)
  }

        ...
        <el-tab-pane label="基本信息" name="info">
          <el-form :model="infoForm" :rules="infoRules" ref="infoFormRef" label-width="100px" style="max-width: 500px">
            ...
            <el-form-item label="审核状态">
              <el-tag :type="infoForm.status === 0 ? 'warning' : infoForm.status === 1 ? 'success' : 'danger'">
                {{ infoForm.status === 0 ? '待审核' : infoForm.status === 1 ? '已通过' : '已拒绝' }}
              </el-tag>
            </el-form-item>
            <el-form-item>
...
const handleUpdateInfo = async () => {
  ...
  loading.value = true
  try {
    await candidateApi.update(infoForm.value)
